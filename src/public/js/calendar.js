...
function showCalendar(data) {
    ...
    for (let i = 1; i <= lastDay; i++) {
        if (i === new Date().getDate() && date.getMonth() === new Date().getMonth()) {
            ...
            if (check) {
                days += `<div class="day"><button type="button" data-toggle="modal" class="showEventBtn" data-target="#showEvent" data-date="${date.getFullYear()}-${date.getMonth() + 1}-${i}" id="today">${i} <div class="event"></div> </button></div>`;
            } else {
            ...
            let check = eventTimes.find(e => e === eventTime);
            if (check) {
                days += `<div class="day"><button type="button" data-toggle="modal" class="showEventBtn" data-target="#showEvent" data-date="${date.getFullYear()}-${date.getMonth() + 1}-${i}">${i} <div class="event"></div></button></div>`;
            } else {
            ...
            xhr.onload = function () {
                if (xhr.status === 200) {
                    ...
                    if (!data.length) {
                        ...
                        document.querySelector('#accordion').innerHTML = '<h5 class="center">Chưa có sự kiện nào được thêm!</h5>';
                    };
...
function addEvent() {
    document.querySelector('#addEventBtn').addEventListener('click', (e) => {
        ...
        const date = dateTimeHelper(new Date(time)).split(" ")[0];

        const data = { title, time, date, noticeTime, description };
        const xhr = new XMLHttpRequest();
